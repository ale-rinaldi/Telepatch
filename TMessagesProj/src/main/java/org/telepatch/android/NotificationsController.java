...
import org.telepatch.ui.MainActivity;
...
public class NotificationsController {
    ...
    private String getStringForMessage(MessageObject messageObject) {
        ...
        String msg = null;
        if ((int)dialog_id != 0) {
            //caso in cui il messaggio non e' in un gruppo
            if (chat_id == 0 && user_id != 0) {
                ...
                //caso in cui il nuovo messaggio e' in un gruppo
            } else if (chat_id != 0) {
                    ...
                    if (messageObject.messageOwner instanceof TLRPC.TL_messageService) {
                        ...
                        if (messageObject.messageOwner.media instanceof TLRPC.TL_messageMediaEmpty) {
                            if (messageObject.messageOwner.message != null && messageObject.messageOwner.message.length() != 0) {
                                ...
                                msg = LocaleController.formatString("NotificationMessageGroupText", R.string.NotificationMessageText, Utilities.formatName(user.first_name, user.last_name), chat.title, messageObject.messageOwner.message);
                            } else {
    ...
    //TODO qui lui crea le notifiche
    private void showOrUpdateNotification(boolean notifyAboutLast) {
        ...
        try {
            ...
            */
            ...
            //TODO qui aggiungo il supporto ad Android Wear

            String replyLabel = ApplicationLoader.applicationContext.getResources().getString(R.string.reply);
            RemoteInput remoteInput = new RemoteInput.Builder(EXTRA_VOICE_REPLY)
                    .setLabel(replyLabel)
                    .build();

            PendingIntent replyPendingIntent =
                    PendingIntent.getActivity(ApplicationLoader.applicationContext, 0, intent,
                            PendingIntent.FLAG_UPDATE_CURRENT);

            NotificationCompat.Action action =
                    new NotificationCompat.Action.Builder(R.drawable.telepatch_mini,
                            ApplicationLoader.applicationContext.getString(R.string.reply), replyPendingIntent)
            ...
            if (chat_id != 0) {
                name = LocaleController.formatString("NotificationMessageGroupText", R.string.NotificationMessageGroupText, Utilities.formatName(user.first_name, user.last_name), chat.title);
            ...
            */
            ...
            if (!preferences.getBoolean("multiple_notify", true)) {
                ...
                detailText = lastMessageObject.messageText.toString();
            }

            boolean quickReplyEnabled = preferences.getBoolean("QuickReplyEnabled", true);
            ...
            if (quickReplyEnabled) {
                Intent quickIntent = new Intent(ApplicationLoader.applicationContext, MainActivity.class);

                if ((int) dialog_id != 0) {
                    if (chat_id != 0) {
                        quickIntent.putExtra("chatId", chat_id);
                    } else if (user_id != 0) {
                        quickIntent.putExtra("userId", user_id);
                    }
                    quickIntent.putExtra("dialogId", (int) dialog_id);
                    quickIntent.setFlags(32768);
                    quickIntent.putExtra("isQuickReply", true);
            ...
            mBuilder.setStyle(new NotificationCompat.BigTextStyle().bigText(lastMessageObject.messageText));
            ...
            */
            ...
            if (preferences.getBoolean("multiple_notify", true)) {
                notificationManager.notify((int) dialog_id, mBuilder.build());
            } else {
